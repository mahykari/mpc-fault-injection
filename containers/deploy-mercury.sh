...
#
#   ./containers/deploy-mercury.sh [max_runs]        # default 500000
...
ssh "$HOST" "cd $DEST && mkdir -p runs && \
  nohup bash containers/_remote-run.sh $MAX_RUNS > runs/continuous.log 2>&1 < /dev/null &"
