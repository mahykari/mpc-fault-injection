...
**How to apply:** when in doubt, suggest. When asked to write, write the minimum. "Let's do that" / "let's X" / "we should do Y" are *task-starting* phrases, not write permission — the user is naming the task, not delegating it. Wait for an explicit "write it" / "do it for me" / "fix this for me" before touching files. See also [[feedback_yap_less]], [[feedback_code_terseness]].
